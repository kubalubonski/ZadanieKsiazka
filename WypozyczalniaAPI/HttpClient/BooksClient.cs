...
public class BooksClient : IBooksClient
{
    ...
    public async Task<BookDto> GetBookById(int bookId)
    {
       ...
       _logger.LogInformation($"Request action  with path: {_bookUrl}/{bookId} and bookId: {bookId}, X-Request-ID: {requestId}  GET_BOOK_BY_ID action invoked");
        ...
        _logger.LogInformation($"Request action  with path: {_bookUrl}/{bookId} and bookId: {bookId}, X-Request-ID: {requestId}  GET_BOOK_BY_ID action executed ");
